...
def RK78(X0, T0, T1, f, params):
    if isinstance(
        X0, array
    ):  # daceypy.array is a subclass of np.ndarray, so both return true. Need to make this check first.p
        return _RK78_dace(X0, T0, T1, f, params)
    elif isinstance(X0, np.ndarray):
        return _RK78_numpy(X0, T0, T1, f, params)
    else:
        raise TypeError("Unsupported input type for RK78")


def _RK78_dace(
    X0: array, T0: float, T1: float, f: Callable[[float, array], array], params
):
    ...
    while T != T1:
        ...
        for j in range(13):
            ...
            for i in range(N):
                ...
                for k in range(j):
                    X0[i] = X0[i] + Z[i, k + 3] * B[j, k]

                X0[i] = H * X0[i] + Z[i, 0]

            Y1 = f(T + H * A[j], X0, params)
...
def _RK78_numpy(
    ...
    while T != T1:
        if RFNORM != 0:
            H *= min(4.0, np.exp(HSQR * np.log(EPS / RFNORM)))
        if abs(H) > HH1:
            H = HH1
        elif abs(H) < HH0 * 0.99:
            H = HH0
        ...
        for i in range(N):
            Z[i, 1] = np.dot(Z[i, 3:16], D)
            Z[i, 2] = np.dot(Z[i, 3:16], C)
            Y1[i] = (Z[i, 2] - Z[i, 1]) * H
            Z[i, 2] = Z[i, 2] * H + Z[i, 0]

        RFNORM = np.linalg.norm(Y1, np.inf)
        if RFNORM > BS and abs(H / H0) > 1.2:
            H = H / 3.0
            RFNORM = 0.0
        else:
