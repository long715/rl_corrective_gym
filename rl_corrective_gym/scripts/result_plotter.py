...
df = pd.read_csv(
    "../../SAC-mars-25_10_21_11-16-58/10/data/eval.csv",
    on_bad_lines="skip",
    engine="python",
)
...
def plot_terminal():
    ...
    for state in df["gui_err"][-1:].to_numpy():
    ...
    for state in df["ngui_err"][-1:].to_numpy():
        # ignore the mass for now
        state_numpy: np.ndarray = np.fromstring(state.strip("[]"), sep=" ")
    ...
    nom_traj: np.ndarray = env.nominal_traj[env.chosen_timestamp]
    ...
    corr_imp: np.ndarray = np.fromstring(df["action"].iloc[-1].strip("[]"), sep=" ")
    total: np.ndarray = corr_imp + nom_imp

    print(total)
    print(np.linalg.norm(total), np.linalg.norm(nom_imp))

    terminal: np.ndarray = env.nominal_traj[-1, :]
    terminal[3:6] -= env.nominal_imp[-1, :]
    ...
    optimal_dev: np.ndarray = env._propagate(True, opt_control + nom_imp) - terminal
...
def plot_trajectory():
    ...
    for i in range(1, 2):
        ...
        nom_traj = env.nominal_traj[env.chosen_timestamp]
        env_imp = env.nominal_imp[env.chosen_timestamp]
        nom_traj[3:6] -= env_imp

        env.state = np.concatenate((nom_traj, env_imp, nom_traj + env.noise))

        imp: np.ndarray = np.fromstring(df["action"].iloc[-i].strip("[]"), sep=" ")
        sum += np.linalg.norm(imp)

        env._init_logs()
        env._propagate(True, imp)
        env._propagate(False)
        ax.plot(
            env.gui_log_pos[:, 0], env.gui_log_pos[:, 1], env.gui_log_pos[:, 2], "r"
        )
        print(np.linalg.norm(env.gui_log_pos[-1, :] - env.nominal_traj[-1, 0:3]))
        ax.plot(
            env.nogui_log_pos[:, 0],
            env.nogui_log_pos[:, 1],
            env.nogui_log_pos[:, 2],
        ...
        print(np.linalg.norm(env.nogui_log_pos[-1, :] - env.nominal_traj[-1, 0:3]))
